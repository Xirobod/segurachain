        ...
        public static ClassEnumLoadArgument HandleArgument(string[] args, out ClassSoloMinerSettingObject soloMinerSettingObject)
        {
            ...
            if (args.Length == ClassEnumIndexArgument.MinArgumentCount)
            {
                ...
                if (ClassSoloMinerSettingFunction.CheckWalletAddressFormat(args[ClassEnumIndexArgument.IndexArgumentWalletAddress]))
                {
                    ...
                    {
                        if (ClassSoloMinerSettingFunction.CheckMaxThread(maxThread, true))
                        {
                            if (int.TryParse(args[ClassEnumIndexArgument.IndexArgumentThreadPriority], out int threadPriority))
                            {
                                if (ClassSoloMinerSettingFunction.CheckThreadPriority(threadPriority, true))
                                {
                                    if (ClassSoloMinerSettingFunction.CheckPeerIpTarget(args[ClassEnumIndexArgument.IndexArgumentPeerIpTarget], true))
                                    {
                                        if (int.TryParse(args[ClassEnumIndexArgument.IndexArgumentPeerApiPortTarget], out int peerApiPortTarget))
                                        {
                                            if (ClassSoloMinerSettingFunction.CheckPeerApiPortTarget(peerApiPortTarget, true))
                                            {
                                                error = false;
